...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  const newArray= palabras.map(item=>item).join(" ");
  return newArray
}
...
function mesesDelAño(array) {
  ...
  let newArray=[];
    ...
    array.map(mes=>{
        if(mes==="Enero" || mes==="Marzo" || mes==="Noviembre"){newArray.push(mes)}
    })
...
function mayorACien(array) {
  ...
  array.map(valor => {
    if(valor>100){newArray.push(valor)}
  });
...
function breakStatement(numero) {
  ...
  const newArray=[];
    ...
    for (let index = 0; index < 10; index++) {
        ...
        if(resultado===index){return "Se interrumpió la ejecución"}
    }
